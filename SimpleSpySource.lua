...
local Mouse = (Players.LocalPlayer or Players:GetPropertyChangedSignal("LocalPlayer"):Wait() or Players.LocalPlayer):GetMouse()
...
function bringBackOnResize()
    ...
    if (currentY < 0) or (currentY > (viewportSize.Y - (closed and 19 or Background.AbsoluteSize.Y))) then
        if currentY < 0 then
            currentY = 0
        else
            currentY = viewportSize.Y - (closed and 19 or Background.AbsoluteSize.Y)
        end
    end
        ...
        if not connections["drag"] then
            connections["drag"] = RunService.RenderStepped:Connect(function()
                ...
                if newPos ~= lastPos then
                    ...
                    if (currentY < 0 and currentY < currentPos.Y) or (currentY > (viewportSize.Y - (closed and 19 or Background.AbsoluteSize.Y)) and currentY > currentPos.Y) then
                        if currentY < 0 then
                            currentY = 0
                        else
                            currentY = viewportSize.Y - (closed and 19 or Background.AbsoluteSize.Y)
...
function toggleMaximize()
    if not sideClosed and not maximized then
        ...
        disable.MouseButton1Click:Connect(function()
            if Mouse.Y >= CodeBox.AbsolutePosition.Y and Mouse.Y <= CodeBox.AbsolutePosition.Y + CodeBox.AbsoluteSize.Y and Mouse.X >= CodeBox.AbsolutePosition.X and Mouse.X <= CodeBox.AbsolutePosition.X + CodeBox.AbsoluteSize.X then
                return
            end
